...
- **`LightCore.tsx`** — **not R3F/3D** (was a WebGL shader plane originally; rewritten to a plain CSS radial-gradient div because the three.js version blocked the main thread 1-1.6s on first Hero mount). Plain eager import (no `next/dynamic`, no `ssr:false` — nothing left to defer). The glow's scroll-driven scale is a compositor-only `transform:scale(var(--lc-scale))` written directly onto a ref by `Hero.tsx`'s own `ScrollTrigger`; color rides the shared `--accent-current` CSS var from `LightTemperatureProvider`. Used by `sections/Hero.tsx`. **Perf note**: the glow previously had an always-on `light-core-pulse` CSS animation (3.2s infinite breathing scale) independent of scroll — profiling under CPU throttling (real headless-Chrome measurement, not guesswork) showed this was the dominant cause of scroll jank on mid/low-end mobile (avg fps ~18→~54 at 4x throttle, ~4→~41 at 6x, in a production build), far more than the hero text's `drop-shadow` filters. The animation was removed for exactly this reason — don't re-add a continuous background animation here without re-measuring.

Pattern: 3D Canvases (`Globe.tsx` and the three brand `*Scene.tsx` files — **not** `LightCore.tsx`, see above) are always `next/dynamic({ssr:false})`, mounted as fixed/absolute full-bleed backgrounds, never imported eagerly.
...
Bottom-left floating widget, mounted once in the root `layout.tsx` so it appears on every page. Deliberately **not** a real chat — it's a canned decision-tree FAQ with a typing-indicator delay to feel conversational.
- **`ChatbotWidget.tsx`** — all UI/state. Detects brand context client-side (no props from layout) via `window.location.hostname` prefix (`k2.`/`vanti.`/`global.`) for local dev + production subdomains, falling back to matching `/brand/(k2|vanti|global)` in `window.location.pathname` for the GitHub Pages path-based case where there's no subdomain. Recolors itself per brand (`ACCENTS` map) and flips the panel between a light theme (on the dark main site) and a dark theme (on the light-themed brand pages) via `isLightPanel = accentKey === 'main'`.
  - Panel persists in the DOM after first open (`hasMounted`) and is hidden via opacity/transform + `pointer-events-none` rather than unmounted, so reopening is instant and conversation state survives.
...
- **`chatbotContent.ts`** — the actual Q&A data (`CHAT_NODES`, bilingual tr/en), kept out of `ui/i18n/*.json` since it's sizeable, self-contained, bespoke copy (same reasoning as the `brand/*CreativePage.tsx` inline-translations pattern below). Cross-brand links use `getBrandExternalHref()` (`src/lib/basePath.ts`) — for linking to a brand's own site from *outside* it, unlike `getBrandHomeHref`/`getBrandUrunlerHref` which assume you're already on that brand's origin.
...
- **`Hero.tsx`** — `h-[130vh]` sticky hero hosting `LightCore` (CSS glow, not 3D — see `engine/` above) as background, tracks scroll progress into a ref.
...
- **`LightTemperatureProvider.tsx`** — global `ScrollTrigger` over `document.body` (`scrub:true`), lerps a cool blue (`#d8e4ff`) → warm orange (`#ffb347`) `THREE.Color` and writes CSS vars `--light-temp`/`--accent-current`. Rounds progress to 1/500 to reduce redundant DOM writes. `useLightTemperature()` exposes `getProgress()`. Wraps app in root layout; consumed by `Globe.tsx` (via the hook) and by `LightCore.tsx`'s CSS (`.light-core-glow`'s `background` reads `var(--accent-current)` directly, no hook/JS involved).
- **`useInView.ts`** — `useInView<T extends HTMLElement>(rootMargin="200px")`, `IntersectionObserver`-based, returns `[ref, isInView]` (defaults `isInView=true`). Used by `Globe.tsx` for frameloop gating (not `LightCore.tsx` — it isn't a Canvas, has no frameloop to gate).
...
2. **3D Elements:** R3F scenes (`Globe` and the three brand `*Scene.tsx` files) are always `dynamic({ssr:false})`, full-bleed backgrounds, frameloop-gated for perf. `LightCore` (homepage hero) looks like it belongs on this list but is plain CSS, not R3F — see `engine/` above.
