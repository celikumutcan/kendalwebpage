...
- **Özellikleri Birleştirme:** Kullanıcının JSON'ında `Özellik`, `Açıklama`, `Garanti`, `IP Sınıfı` (IP Class) gibi dağınık anahtarlar olabilir. Bunların **tamamını** `Özellik` (İngilizce ise `Feature`) adı altında, aralarına `" / "` koyarak birleştirmelisin. (Örn: `"Kompakt Driver Özelliği / Yerli Üretim / 5 Yıl Garanti / IP64"`).
