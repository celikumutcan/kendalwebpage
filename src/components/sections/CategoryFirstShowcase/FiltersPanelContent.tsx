  ...
  return (
    <>
      <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-100">
        <div>
          <h3 className="text-lg font-bold text-zinc-900 tracking-tight">
            {t.brand_pages?.showcase?.filters || 'Filtreler'}
          </h3>
      ...
      <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-zinc-200">
        {activeFilterTab === 'casings' &&
          availableFilters.casings.length > 0 && (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5 pr-1">
                {availableFilters.casings
                  ...
                  .map((casing) => (
                    <label
                      key={casing}
                      className={`flex items-center gap-3 p-3.5 rounded-2xl cursor-pointer transition-all duration-200 border ${selectedCasings.includes(casing) ? (isK2 ? 'border-orange-500/60 bg-orange-50 shadow-sm' : 'border-blue-500/60 bg-blue-50 shadow-sm') : 'border-zinc-200 hover:border-zinc-300 hover:bg-zinc-50'}`}
                    >
                      <div
                      ...
                      <span
                        className={`text-sm flex-1 ${selectedCasings.includes(casing) ? 'font-semibold text-zinc-900' : 'text-zinc-600'}`}
                      >
                        {casing}
                      </span>
                      <input
                        ...
                        onChange={() => {
                          setSelectedCasings((prev) =>
                            prev.includes(casing)
                              ? prev.filter((c) => c !== casing)
                              : [...prev, casing],
                          );
                          setCurrentPage(1);
                        }}
                ...
                {availableFilters.casings.filter((c) =>
                  c.toLowerCase().includes(casingSearchQuery.toLowerCase()),
                ).length === 0 && (
            ...
            <div className="flex flex-wrap gap-2.5">
              {availableFilters.watts.map((watt) => (
                <button
                  key={watt}
                  onClick={() => {
                    setSelectedWatts((prev) =>
                      prev.includes(watt)
                        ? prev.filter((w) => w !== watt)
                        : [...prev, watt],
                    );
                    setCurrentPage(1);
                  }}
                  className={`px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-200 border ${
                    selectedWatts.includes(watt)
                      ? isK2
                        ? 'bg-orange-600 text-white border-orange-600 shadow-md shadow-orange-600/20 scale-[1.03]'
                        : 'bg-blue-600 text-white border-blue-600 shadow-md shadow-blue-600/20 scale-[1.03]'
        ...
        {activeFilterTab === 'sockets' &&
          availableFilters.sockets.length > 0 && (
            <div className="animate-in fade-in zoom-in-95 duration-200">
              <div className="flex flex-wrap gap-2.5">
