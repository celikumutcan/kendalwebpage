...
export function ProductCard({
  ...
  const blobClass = isK2 ? "bg-orange-200/50" : isVanti ? "bg-blue-200/50" : "bg-amber-200/50";
  ...
  const borderHoverClass = isK2 ? "hover:border-orange-200" : isVanti ? "hover:border-blue-200" : "hover:border-amber-200";
  ...
  return (
    ...
    >
      <div className="relative aspect-square flex items-center justify-center overflow-hidden bg-zinc-50/40">
        <div className={`absolute w-2/3 h-2/3 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${blobClass}`} />
        ...
        {canCompare && (
          <button
            ...
            className={`absolute top-3 left-3 z-20 flex items-center gap-1.5 pl-1.5 pr-2.5 py-1.5 rounded-full text-[11px] font-bold border shadow-sm transition-all duration-200 ${
              isCompared
                ? (isK2 ? "bg-orange-500 border-orange-500 text-white" : isVanti ? "bg-blue-600 border-blue-600 text-white" : "bg-zinc-900 border-zinc-900 text-white")
                : isCompareMaxed
        ...
        <span className={`absolute bottom-3 right-3 z-20 w-9 h-9 rounded-full text-white flex items-center justify-center scale-0 group-hover:scale-100 transition-transform duration-300 ease-out shadow-lg ${ctaClass}`}>
      ...
      <div className="p-5 pt-4 flex flex-col flex-grow border-t border-zinc-50">
        <div className="text-[11px] font-bold text-zinc-400 mb-1.5 tracking-wider uppercase">{showcaseTexts.model} {product.model}</div>
        <h4 className="font-bold text-sm leading-snug line-clamp-2 text-zinc-800" title={displayName}>
          {displayName}
        </h4>
      </div>
    </Link>
