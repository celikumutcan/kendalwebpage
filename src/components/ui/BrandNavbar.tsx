...
export const BrandNavbar = ({ brandName }: BrandNavbarProps) => {
  ...
  const theme = brandThemes[brandName as keyof typeof brandThemes] || brandThemes.k2;
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between w-full relative">
          <Link href={homeHref} className="relative flex items-center hover:opacity-80 transition-all duration-300 z-50 hover:scale-105">
            <Image 
              src={logoSrc} 
              alt={`${brandName} Logo`} 
              width={140} 
          ...
          <div className="hidden lg:flex items-center gap-2 text-[15px] font-semibold absolute left-1/2 -translate-x-1/2">
            <Link href={homeHref} className={`px-4 py-2 rounded-full transition-all duration-300 text-zinc-600 ${theme.hoverText} ${theme.hoverBg}`}>
              {(t as any).brand_pages?.navbar?.home || "Ana Sayfa"}
            </Link>
            <Link href={urunlerHref} className={`px-4 py-2 rounded-full transition-all duration-300 text-zinc-600 ${theme.hoverText} ${theme.hoverBg}`}>
      ...
      >
        <div className={`absolute inset-0 ${theme.mobileBg} backdrop-blur-2xl`} onClick={() => setIsMobileMenuOpen(false)} />
        <div className={`absolute top-[80px] left-0 w-full flex flex-col p-8 gap-6 text-center text-xl font-bold transition-all duration-500 delay-100 ${
          isMobileMenuOpen ? "translate-y-0 opacity-100" : "-translate-y-8 opacity-0"
        }`}>
          <Link href={homeHref} className="text-white/80 hover:text-white transition-colors py-2 border-b border-white/10" onClick={() => setIsMobileMenuOpen(false)}>
            {(t as any).brand_pages?.navbar?.home || "Ana Sayfa"}
          </Link>
          <Link href={urunlerHref} className="text-white/80 hover:text-white transition-colors py-2 border-b border-white/10" onClick={() => setIsMobileMenuOpen(false)}>
            {(t as any).brand_pages?.navbar?.products || "Ürünler"}
          </Link>
          <Link href={`${homeHref}#iletisim`} className={`mt-4 py-4 rounded-2xl flex items-center justify-center gap-2 shadow-xl ${theme.btn}`} onClick={() => setIsMobileMenuOpen(false)}>
            Bize Ulaşın
          </Link>
        </div>
      </div>
