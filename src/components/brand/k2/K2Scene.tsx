  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollY <= introOffset) {
        ...
        k2DawnProgress = Math.min(1, Math.max(0, (scrollY - introOffset) / climbDistance));
      }
    };
...
function Sunrise() {
  ...
  return (
    <>
      ...
      <pointLight ref={glowLightRef} color="#ffb35c" position={[3.4, -1.6, -6.5]} distance={32} decay={2} intensity={0} />
      <directionalLight ref={rimLightRef} color="#ffcf9c" position={[3.4, 2, -3]} intensity={0.12} />
    </>
  );
}
    ...
    <group>
      ...
      {snowCap && (
        ...
        >
          ...
          <meshStandardMaterial color="#eef3f8" roughness={0.55} metalness={0.05} flatShading />
        </mesh>
      )}
    </group>
  );
...
function MountainRange() {
  ...
  useFrame(() => {
    if (groupRef.current) {
      ...
      groupRef.current.rotation.y += (target - groupRef.current.rotation.y) * 0.03;
    }
  });
  ...
  return (
    <group ref={groupRef} position={[0, -2.4, -4]}>
      {/* distant range */}
      <Peak position={[-6.2, -0.7, -6]} height={2.4} radius={2.1} rotation={0.6} color="#242428" snowCap={false} />
      <Peak position={[5.8, -0.9, -6.4]} height={2.1} radius={1.9} rotation={-0.35} color="#242428" snowCap={false} />

      {/* K2 summit */}
      <Peak position={[0, 1.3, 0]} height={5.8} radius={2.7} rotation={0.12} color="#1c1c1f" />

      {/* flanking peaks */}
      <Peak position={[-2.8, 0.1, -1.4]} height={3.4} radius={2} rotation={0.42} color="#19191c" />
      <Peak position={[2.9, -0.2, -1.6]} height={2.9} radius={1.8} rotation={-0.5} color="#19191c" />
    </group>
  );
}
...
function K2Environment() {
  return (
    <>
      ...
      <Sparkles count={90} scale={[24, 10, 14]} size={1.3} speed={0.12} opacity={0.25} color="#ffe4c2" position={[0, 4, -3]} />

      <MountainRange />
      <Sunrise />

      <mesh receiveShadow position={[0, -2.2, -5]} rotation={[-Math.PI / 2, 0, 0]}>
